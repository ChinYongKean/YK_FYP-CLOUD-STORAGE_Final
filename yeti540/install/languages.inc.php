...
if (!isset($arr_active_languages)) {
    $arr_active_languages = [];
}

if (!empty($lang) && array_key_exists($lang, $arr_active_languages)) {
    $curr_lang = $_SESSION['curr_lang'] = $lang;
} else {
    if (isset($_SESSION['curr_lang']) && array_key_exists($_SESSION['curr_lang'], $arr_active_languages)) {
        $curr_lang = $_SESSION['curr_lang'];
    } else {
        $curr_lang = INSTALLER_DEFAULT_LANGUAGE;
    }
}
...
if (file_exists("install/language/".$curr_lang."/common.inc.php")) {
    ...
    if (file_exists("language/".$curr_lang."/common.inc.php")) {
