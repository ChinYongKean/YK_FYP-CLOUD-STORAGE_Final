...
class IndexController extends CoreIndexController
{
    ...
    public function index() {
        ...
        // handle toggling of plugins in demo mode
        if ($this->inDemoMode()) {
    ...
    public function upgrade($packageId = null) {
        ...
        if($request->query->has('e')) {
            NotificationHelper::setError($request->query->get('e'));
        }
        ...
        if ($packageId === null) {
            ...
            if (count($premiumPackages) === 1) {
                foreach ($premiumPackages AS $premiumPackage) {
                    // ensure any errors are appended
                    return $this->redirect(WEB_ROOT . '/upgrade/' . $premiumPackage['id'] . ($request->query->has('e')?'?e='.urlencode($request->query->get('e')):''));
                }
            }
    ...
    public function ajaxSearch() {
        ...
        // load filtered
        $results = $db->getRows('SELECT file.*, fa.file_size '
                . 'FROM file '
        ...
        if ($results) {
            foreach ($results AS $result) {
                ...
                $previewImageUrlMedium = FileHelper::getIconPreviewImageUrl($fileObj, false, 160, false, 68, 68, 'middle');
                ...
                $cell1 .= '<p>' . TranslateHelper::t('search_date_uploaded', 'Dated Uploaded') . ': ' . ValidationHelper::safeOutputToScreen(CoreHelper::formatDate($result['uploadedDate'], SITE_CONFIG_DATE_FORMAT)) . '&nbsp;&nbsp;' . TranslateHelper::t('search_filesize', 'Filesize') . ': ' . ValidationHelper::safeOutputToScreen(CoreHelper::formatSize($result['file_size'])) . '<p>';
                $cell1 .= '</div>';
