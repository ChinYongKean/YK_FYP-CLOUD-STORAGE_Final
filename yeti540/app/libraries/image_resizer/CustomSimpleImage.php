<?php
    ...
    class CustomSimpleImage extends SimpleImage
    {
        ...
        function load_from_image_content($string)
        {
            ...
            // create temp file
            $path = CACHE_DIRECTORY_ROOT . '/plugins/filepreviewer/_tmp';
            if(!file_exists($path))
            {
        ...
        function thumbnail($width, $height = null)
        {
            ...
            // force white background
            return $this->fixTransparentPng($imgObj, $width, $height);
        }
        ...
        function fixTransparentPng($imgObj, $width, $height)
        {
            ...
            //$black = imagecolorallocate($imgObj->image, 0, 0, 0);

            // remove the black
            //imagecolortransparent($imgObj->image, $black);

            // turn off alpha blending
            imagealphablending($imgObj->image, false);
