...
class Auth
{
    ...
    public function impersonate($userIdOrUsername, $accessCode2fa = null, $updateLastLogin = false, $skip2fa = false) {
        ...
        if ($updateLastLogin === true) {
            ...
            AuthHelper::logSuccessfulLogin($user->id, $iPAddress);
        }
    ...
    public function attemptLogin($username, $rawPassword, $sessionLogin = false, $fromLoginForm = false, $accessCode2fa = null, $skip2fa = false) {
        ...
        if ($user->status === "suspended") {
            $this->lastLoginError = 'account_suspended';
        ...
        if ($fromLoginForm) {
            // user action logs
            UserActionLogHelper::log('Account login', 'LOGIN', 'READ', [
                'user_id' => $this->id,
